...
public class KDBX {
    ...
    public static func fromEncryptedData(_ data: Data, password: String) throws -> KDBX {
        let readStream = InputStream(data: data)
        readStream.open()
        let kdbx = try KDBX(readStream, password: password)
        readStream.close()
        return kdbx
    }
    ...
    public func encryptToData(password: String) throws -> Data {
        self.body.loadMeta(self.meta)
        self.body.loadGroup(self.group)
